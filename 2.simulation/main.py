...
sensitivityFig = Figure(figsize=(3, 3), dpi=100)
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = ttk.Label(
            self, text="Are you ready to make some money", font=LARGE_FONT)
...
class ManualTest(tk.Frame):
    def __init__(self, parent, controller):
        ttk.Frame.__init__(self, parent)
        label = ttk.Label(
            self, text="Adjusting simulation Page!", font=LARGE_FONT)
        # label.pack(pady=10, padx=10)
        ...
        button2 = ttk.Button(self, text="Go to SensitivityAnalysis",
                             command=lambda: controller.show_frame(SensitivityAnalysis))
        # button2.pack()
