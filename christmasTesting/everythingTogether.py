...
class MakeTrades():
    ...
    def __init__(self):
        ...
        self.trading = betfairlightweight.APIClient(obj["username"], obj["password"], app_key=obj["key"])
        self.main()
    ...
    def main(self):
        ...
        heappush(self.cancelTradesQueue, (datetime(2022, 3, 20), 123445, 'FALSE'))
    ...
    def waitTillEarliestAction(self):
        ...
        if self.nextToJump  < self.cancelTradesQueue[0][0]:
            print("waiting for next to jump at %s, time now is %s"%(self.nextToJump, datetime.now()))
            timeToSleep = (self.nextToJump - datetime.now() - timedelta(seconds=10))
            ...
            print("waiting for cancel trades at %s, time now is %s"%(self.cancelTradesQueue[0][0], datetime.now()))
            timeToSleep = (self.cancelTradesQueue[0][0] - datetime.now() - timedelta(seconds=10))
            if timeToSleep>timedelta(seconds=0):
                print("sleeping")
                self.sleepCountdown(timeToSleep.seconds)
    ...
    def createMatchTimeList(self, extendedNumber=0):
        ...
        for market_catalogue in market_catalogues:
            ...
            if market_catalogue.market_start_time+self.timeZoneCorrection > datetime.now()+timedelta(seconds=30):
                ...
                temp[1] = market_catalogue.market_start_time+self.timeZoneCorrection
                self.matchTimeList.append(temp.copy())
            else:
                print("didn't make it %s"%(market_catalogue.market_start_time+self.timeZoneCorrection))
    ...
    def updateCancelTrades(self, market_id):#, bet_id):
        ...
        heappush(self.cancelTradesQueue, (datetime.now() + timedelta(seconds=40), market_id, 'FALSE'))
        print("pushed %s to cancel queue, queue looks like this: %s"%(market_id, list(self.cancelTradesQueue)))
    ...
    def cancelTrades(self):
        ...
        marketBook = self.trading.betting.list_market_book([str(needToCancel[1])])[0]
        ...
        if marketBook.inplay == False:
            ...
            if matchTime-datetime.now() > timedelta(minutes=5):
                print("****************** we reset the old time to %s"%(marketCatalogue.market_start_time))
                #CANCEL THE TRADE
                report = self.trading.betting.cancel_orders(needToCancel[1])
                print("cancelled a bet in %s market, result is %s"%(report.market_id, report.status))
                self.nextToJump = datetime.now()
    ...
    def getMatchList(self, extendedNumber=0):
        ...
        while True:
            ...
            event = self.trading.betting.list_event_types(filter=sport_market_filter)[0]
            ...
            market_catalogues = self.trading.betting.list_market_catalogue(
                ...
                filter=filters.market_filter(
                    ...
                    market_type_codes=['MATCH_ODDS'],  # filter on just MATCH_ODDS market types
                    in_play_only=False             # filter on just matches that haven't started
                ),

                market_projection=['MARKET_START_TIME', 'RUNNER_DESCRIPTION'],  # start time and runner description required
